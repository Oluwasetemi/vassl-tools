...
VASSL is built exclusively for Video Access Security Solutions Ltd internal staff. It is not a public product. Each person who uses the application has their own account with specific permissions controlling which modules they can access.

## User accounts

VASSL requires a login every time it opens. An **administrator** account is created the very first time VASSL is launched on a machine. Administrators can then create additional accounts for other staff members and control exactly what each person can do.
