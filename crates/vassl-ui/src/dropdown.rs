use gpui::{Context, Entity, EventEmitter, FocusHandle, Focusable, IntoElement, KeyDownEvent,
           MouseButton, MouseDownEvent, Render, SharedString, Window, actions, deferred, div,
           prelude::*, px, rems, rgb};
...
actions!(dropdown, [DropdownDown, DropdownUp, DropdownConfirm, DropdownClose]);
...
impl Dropdown {
    ...
    fn close(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some(si) = &self.search_input {
            ...
            si.update(cx, |t, cx| { t.show_cursor = false; t.reset(cx); });
        }
        self.recompute_filtered();
        ...
        div()
            ...
            .on_key_down(cx.listener(|this, event: &KeyDownEvent, _w, cx| {
                ...
                match event.keystroke.key.as_str() {
                    "backspace" => {
                        let current = this.search_input.as_ref()
            ...
            .child(
                div()
                    ...
                    .w_full().min_w(px(0.)).overflow_hidden()
                    .flex().flex_row().items_center().gap(px(6.))
                    .px(px(12.)).py(px(7.))
                    .bg(rgb(c.surface_default)).rounded(px(5.))
                    .when( show_active_border, |d| d.border_2().border_color(rgb(c.surface_active)))
                    .when(!show_active_border, |d| d.border_1().border_color(rgb(c.surface_active)))
                    .cursor_pointer()
                    ...
                    .child(
                        div().flex_1().min_w(px(0.)).text_size(rems(0.923))
            ...
            .when(is_open, move |d| {
                let search_row = search_input.as_ref().map(|si| {
                    let border_c = if popup_is_focused { c.surface_active } else { c.canvas_bg };
                    div()
                        .w_full()
                        .px(px(8.)).py(px(6.))
                ...
                let item_list = {
                    let base = div()
                        .id("dropdown-items")
                        .w_full().max_h(px(160.)).overflow_y_scroll();
                    if loading {
                        base.child(
                        ...
                        let msg = if query.is_empty() { empty_message }
                                  else { format!("No results for \"{query}\"") };
                        base.child(
                            div().px(px(10.)).py(px(8.))
                                .text_size(rems(0.923)).text_color(rgb(c.text_muted))
                                .child(msg)
                        ...
                        base.children(filtered_owned.into_iter().enumerate().map(|(i, item)| {
                            ...
                            let bg           = if selected        { c.surface_active }
                                              else if is_kb_focus { c.surface_hover  }
                                              else                { c.surface_default };
                            let hover_bg     = rgb(c.surface_hover);
                            let text = SharedString::from(
                                item.sublabel.as_ref()
                                    .map_or_else(|| item.label.clone(), |sub| format!("{} · {}", item.label, sub))
                            );
                            div()
                                .id(SharedString::from(format!("dropdown-item-{id}")))
                                .w_full().h(px(30.)).overflow_hidden()
                                .flex().flex_row().items_center()
                                .px(px(10.))
                                .bg(rgb(bg)).cursor_pointer()
                                .when(!selected && !is_kb_focus, |d| d.hover(move |s| s.bg(hover_bg)))
                ...
                if let Some(row) = search_row { popup = popup.child(row); }
