...
fn validate_price_entry(qty: &str, cost: &str, markup: &str) -> Result<(f64, f64, f64), String> {
    let q: f64 = qty
        .trim()
        .parse()
        .map_err(|_| "Quantity must be a number > 0".to_string())?;
    if q <= 0.0 {
    ...
    if m <= 0.0 {
        return Err("Markup % must be > 0".to_string());
    }
    Ok((q, c, m))
}
            ...
            Ok((qv, cv, mv)) => {
                ...
                cx.spawn(async move |this, cx| {
                    let result = if let Some(entry_id) = edit_id {
                        db.update_price_entry(entry_id, qv, cv, duty_usd, mv, sell, None, &currency)
                            .await
                            .map(|_| entry_id)
                    } else {
                        db.insert_entry(pid, qv, cv, duty_usd, mv, sell, None, &currency)
                            .await
                    };
...
impl Focusable for PriceEntryForm {
    fn focus_handle(&self, cx: &gpui::App) -> FocusHandle {
        self.cost.read(cx).focus_handle.clone()
    }
}
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .child({
                                                let usd_bg = if !is_jmd {
                                                    c.surface_active
                                                } else {
                                                    c.surface_default
                                                };
                                                let jmd_bg = if is_jmd {
                                                    c.surface_active
                                                } else {
                                                    c.surface_default
                                                };
...
mod tests {
    ...
    fn rejects_zero_quantity() {
        assert!(validate_price_entry("0", "100", "30").is_err());
    }
    #[test]
    fn rejects_empty_cost() {
        assert!(validate_price_entry("1", "", "30").is_err());
    }
    #[test]
    fn rejects_negative_cost() {
        assert!(validate_price_entry("1", "-1", "30").is_err());
    }
    #[test]
    fn rejects_zero_markup() {
        assert!(validate_price_entry("1", "100", "0").is_err());
    }
    #[test]
    fn rejects_negative_markup() {
        assert!(validate_price_entry("1", "100", "-5").is_err());
    }
    #[test]
    fn accepts_valid() {
        assert!(validate_price_entry("10", "100.0", "30.0").is_ok());
    }
    #[test]
    fn forty_pct_duty_calculation() {
