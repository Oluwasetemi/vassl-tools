use gpui::{
    Context, Entity, EventEmitter, IntoElement, Render, SharedString, Subscription, Window,
    div, img, prelude::*, px, relative, rgb, rgba, rems,
};
...
use crate::status_bar::{BADGE_BLUE, BADGE_AMBER, BADGE_GREEN, BADGE_RED};
...
enum UpdateAction {
    None,
    Check,
    Download,   // status is Available — triggers download via InstallUpdate handler
    Restart,    // status is ReadyToInstall — triggers install+restart
}
        ...
        let (status_text, status_color, btn_label, btn_color, btn_action): (SharedString, u32, Option<&'static str>, u32, UpdateAction) =
            match &update_status {
                ...
                UpdateStatus::Available(info) => (
                    ...
                    BADGE_BLUE, Some("Download"), BADGE_BLUE, UpdateAction::Download,
                ),
                UpdateStatus::Downloading { pct } => (
                    format!("Downloading…  {pct}%").into(),
                    BADGE_BLUE, None, 0, UpdateAction::None,
                ),
                UpdateStatus::ReadyToInstall(_) => (
                    "Ready to install".into(),
                    BADGE_AMBER, Some("Restart to Update"), BADGE_AMBER, UpdateAction::Restart,
                ),
                UpdateStatus::Installing =>
                    ("Installing…".into(), BADGE_AMBER, None, 0, UpdateAction::None),
                UpdateStatus::Error(e) => (
                    format!("Error: {e}").into(),
                    BADGE_RED, Some("Retry"), BADGE_BLUE, UpdateAction::Check,
                ),
            };

        // Progress bar — uses relative() so it truly tracks the card's inner width.
        let progress_bar: Option<gpui::AnyElement> = if let UpdateStatus::Downloading { pct } = &update_status {
            let frac = (*pct as f32 / 100.0).clamp(0.0, 1.0);
            Some(
                div().w_full().h(px(4.)).rounded_full().bg(rgb(c.surface_hover)).overflow_hidden()
                    .child(div().h_full().rounded_full().w(relative(frac)).bg(rgb(BADGE_BLUE)))
                    .into_any_element()
            )
        } else {
            None
        ...
        let update_row = {
            ...
            top = top.child(
                div().flex().flex_row().items_center().gap(px(6.))
                    .child(div().w(px(6.)).h(px(6.)).rounded_full().bg(rgb(status_color)))
                    .child(
            ...
            row = row.child(top);
            if let Some(bar) = progress_bar { row = row.child(bar); }
            row
        };
        ...
        let card = div()
            ...
            .child(
                div()
                    ...
                    .mb(px(6.))
                    .child(
                        div().text_size(rems(0.769)).text_color(rgb(BADGE_BLUE)).child("Commit")
                    )
                    .child(
                        div()
            ...
            .child(
                div()
                    ...
                    .mb(px(16.))
                    .child(
                        div().text_size(rems(0.769)).text_color(rgb(BADGE_BLUE)).child("Version")
                    )
                    .child(
                        div()
