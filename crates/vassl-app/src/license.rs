...
impl Edition {
    fn to_byte(&self) -> u8 {
        match self { Self::Alpha => 0, Self::Beta => 1, Self::Pro => 2 }
    }

    fn from_byte(b: u8) -> Option<Self> {
        match b { 0 => Some(Self::Alpha), 1 => Some(Self::Beta), 2 => Some(Self::Pro), _ => None }
    }
}
...
fn encode_payload(edition: &Edition, expiry: Option<chrono::NaiveDate>) -> [u8; 4] {
    ...
    [edition.to_byte(), (days >> 16) as u8, (days >> 8) as u8, days as u8]
}
...
pub fn validate_key(raw: &str) -> Result<LicenseInfo, LicenseError> {
    ...
    if bytes.len() < 4 + HMAC_BYTES { return Err(LicenseError::InvalidFormat); }
...
pub fn build_expired() -> bool {
    let Some(date_str) = option_env!("VASSL_BUILD_EXPIRES") else { return false; };
    let Ok(expiry) = chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d") else { return false; };
...
fn hmac_sha256(data: &[u8], key: &[u8]) -> Vec<u8> {
    ...
    let mut k = [0u8; BLOCK];
    if key.len() <= BLOCK { k[..key.len()].copy_from_slice(key); } else { let h = sha256(key); k[..32].copy_from_slice(&h); }
    let mut ipad = [0x36u8; BLOCK]; let mut opad = [0x5cu8; BLOCK];
    for i in 0..BLOCK { ipad[i] ^= k[i]; opad[i] ^= k[i]; }
    let mut inner = ipad.to_vec(); inner.extend_from_slice(data);
...
fn base32_encode(data: &[u8]) -> String {
    ...
    if bits > 0 { out.push(BASE32_ALPHABET[((buf << (5 - bits)) & 0x1f) as usize] as char); }
    out
}
...
fn base32_decode(s: &str) -> Option<Vec<u8>> {
    ...
    let mut out  = Vec::new();
    for ch in s.chars() {
        let val = BASE32_ALPHABET.iter().position(|&b| b == ch.to_ascii_uppercase() as u8)? as u64;
        buf = (buf << 5) | val;
        bits += 5;
...
mod tests {
    ...
    fn generated_key_has_correct_format() {
        ...
        assert!(groups.iter().all(|g| g.len() == 5), "each group should be 5 chars");
    }
    ...
    fn tampered_key_rejected() {
        ...
        assert_eq!(validate_key(&key).unwrap_err(), LicenseError::InvalidSignature);
    }
