...
use vassl_inventory::importer::{parse_xlsx, ImportRow, ImportSummary};
    ...
    for row in &rows {
            ...
            Some(sup_name) => {
                if let Some(&id) = supplier_cache.get(sup_name) {
                    ...
                    let id = if let Some(id) = existing {
                        ...
                        match sup_db
                            .insert_supplier(sup_name, None, None, None, None, None)
                            .await
                        {
                            Ok(id) => {
        ...
        let product_id = if let Some(existing) = existing {
            match inv_db
                .update_product(
                    ...
                    supplier_id,
                    row.model_number.as_deref(),
                    row.part_number.as_deref(),
                    row.duty_percent,
                    existing.end_of_life,
                    existing.replacement.as_deref(),
                )
                .await
            {
                Ok(_) => {
        ...
        if let Some(cost) = row.cost {
                ...
                .await
            {
                ...
                Err(e) => summary
                    .errors
                    .push(format!("price entry '{}': {e}", row.sku)),
            }
        }
    }
