...
use vassl_core::{Project, Supplier};
...
pub fn build_hits(
    ...
    for p in products {
        if p.product.name.to_lowercase().contains(&q) || p.product.sku.to_lowercase().contains(&q) {
            hits.push(SearchHit {
    ...
    for p in projects {
        if p.name.to_lowercase().contains(&q) || p.client_name.to_lowercase().contains(&q) {
            hits.push(SearchHit {
                module: ActiveModule::Quotations,
                kind: SearchResultKind::Project {
                    id: p.id,
                    client: p.client_name.clone(),
                },
                label: p.name.clone(),
...
impl GlobalSearch {
    pub fn new(cx: &mut Context<Self>) -> Self {
        Self {
            query: cx
                .new(|cx| TextInput::with_placeholder("Search products, suppliers, projects…", cx)),
            selected_idx: 0,
...
mod tests {
    ...
    fn matches_product_by_sku() {
        let hits = build_hits(
            "cam-",
            &[
                make_pws(1, "CAM-001", "IP Camera"),
                make_pws(2, "NVR-001", "NVR"),
            ],
            &[],
            &[],
        );
    ...
    fn matches_supplier_by_name() {
        let hits = build_hits(
            "acme",
            &[],
            &[make_supplier(1, "Acme Ltd", Some("a@acme.com"))],
            &[],
        );
        assert_eq!(hits.len(), 1);
    ...
    fn matches_project_by_client() {
        let hits = build_hits(
            ...
            &[],
            &[],
            &[make_project(1, "CCTV Install", "Sony Corp")],
        );
        assert_eq!(hits.len(), 1);
