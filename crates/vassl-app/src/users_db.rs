...
impl UsersDb {
    pub fn user_count(&self) -> anyhow::Result<i64> {
        let rows = self.select::<i64>("SELECT COUNT(*) FROM users")
    ...
    pub fn list_users(&self) -> anyhow::Result<Vec<AuthUser>> {
        ...
        .map(|rows| {
            rows.into_iter().map(|(id, username, is_admin, ci, cp, cq, ad, ap, mcp, active)| AuthUser {
    ...
    pub async fn verify_credentials(&self, username: String, password: String) -> anyhow::Result<Option<AuthUser>> {
    ...
    pub async fn insert_user(
        ...
        self.write(move |conn| {
            ...
            ((username, hash, can_inventory, can_pricebook, can_quotations, allow_delete, allow_price_edit, now))
    ...
    pub async fn update_user_permissions(
        ...
        self.write(move |conn| {
            ...
            ((can_inventory, can_pricebook, can_quotations, allow_delete, allow_price_edit, id))
    ...
    pub async fn reset_password(&self, id: i64, new_password: &str) -> anyhow::Result<()> {
        ...
        self.write(move |conn| {
            ...
            .context("prepare reset_password")?
            ((hash, id))
    ...
    pub async fn change_password(
        ...
        let stored_rows = self.select_bound::<i64, String>(
            "SELECT password_hash FROM users WHERE id=?1",
        )
        .context("prepare read hash")?
        (id)
        ...
        self.write(move |conn| {
            ...
            .context("prepare change_password")?
            ((new_hash, id))
    ...
    pub async fn reactivate_user(&self, id: i64) -> anyhow::Result<()> {
        self.write(move |conn| {
            conn.exec_bound::<i64>("UPDATE users SET is_active=1 WHERE id=? AND is_admin=0")
                .context("prepare reactivate")?
                (id)
                .context("execute reactivate")
        }).await
    }

    pub async fn log_auth_event(&self, user_id: i64, action: &str, username: &str) -> anyhow::Result<()> {
