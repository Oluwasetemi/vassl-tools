...
impl ProjectForm {
    pub fn new(store: Entity<QuotationStore>, cx: &mut Context<Self>) -> Self {
        Self {
            ...
            client_address: cx
                .new(|cx| TextInput::with_placeholder("e.g. 12 Main St, Kingston (optional)", cx)),
            client_attn: cx.new(|cx| TextInput::with_placeholder("e.g. Jane Smith (optional)", cx)),
            client_tel: cx
                .new(|cx| TextInput::with_placeholder("e.g. 876-555-0100 (optional)", cx)),
            date_started: cx
                .new(|cx| TextInput::with_placeholder("e.g. 2026-01-15 (optional)", cx)),
            date_completed: cx
                .new(|cx| TextInput::with_placeholder("e.g. 2026-06-30 (optional)", cx)),
            technicians: cx
                .new(|cx| TextInput::with_placeholder("e.g. John, Maria, Bob (optional)", cx)),
            client_contact: cx
                .new(|cx| TextInput::with_placeholder("client-side contact (optional)", cx)),
            vassl_contact: cx
                .new(|cx| TextInput::with_placeholder("VASSL-side contact (optional)", cx)),
            signedoff_date: cx
                .new(|cx| TextInput::with_placeholder("e.g. 2026-07-01 (optional)", cx)),
    ...
    pub fn edit(
        store: Entity<QuotationStore>,
        project: &vassl_core::Project,
        cx: &mut Context<Self>,
    ) -> Self {
        let date_started =
            cx.new(|cx| TextInput::with_placeholder("e.g. 2026-01-15 (optional)", cx));
        let date_completed =
            cx.new(|cx| TextInput::with_placeholder("e.g. 2026-06-30 (optional)", cx));
        let technicians =
            cx.new(|cx| TextInput::with_placeholder("e.g. John, Maria, Bob (optional)", cx));
        let client_contact =
            cx.new(|cx| TextInput::with_placeholder("client-side contact (optional)", cx));
        let vassl_contact =
            cx.new(|cx| TextInput::with_placeholder("VASSL-side contact (optional)", cx));
        let signedoff_date =
            cx.new(|cx| TextInput::with_placeholder("e.g. 2026-07-01 (optional)", cx));

        if let Some(v) = &project.date_started {
            date_started.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        if let Some(v) = &project.date_completed {
            date_completed.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        if let Some(v) = &project.technicians {
            technicians.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        if let Some(v) = &project.client_contact {
            client_contact.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        if let Some(v) = &project.vassl_contact {
            vassl_contact.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        if let Some(v) = &project.signedoff_date {
            signedoff_date.update(cx, |t, cx| t.set_text(v.clone(), cx));
        }
        ...
        Self {
            ...
            client_address: cx.new(|cx| {
                TextInput::with_text(project.client_address.clone().unwrap_or_default(), cx)
            }),
            client_attn: cx.new(|cx| {
                TextInput::with_text(project.client_attn.clone().unwrap_or_default(), cx)
            }),
            client_tel: cx
                .new(|cx| TextInput::with_text(project.client_tel.clone().unwrap_or_default(), cx)),
            date_started,
            date_completed,
            technicians,
            client_contact,
                    ...
                    None => {
                        cx.spawn(async move |this, cx| {
                            let result = db
                                ...
                                .await;
                            if let Err(e) = result {
                                tracing::error!("insert_project failed: {e:?}");
                                return Ok(());
                            }
...
fn field_row(
    ...
    div()
        ...
        .child(
            div()
                .flex_1()
                .child(text_field("", input, focused, error, cx)),
        )
}
...
mod tests {
    ...
    fn rejects_empty_name() {
        assert!(validate_project("", "Acme").is_err());
    }
    #[test]
    fn rejects_empty_client() {
        assert!(validate_project("Alpha", "").is_err());
    }
    #[test]
