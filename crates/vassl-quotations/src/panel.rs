...
use vassl_ui::{AppSettings, NewRecord, ThemeHandle, tooltip, tooltip_keyed};
...
enum Tab { Quotations, Items, Projects }
...
impl QuotationPanel {
    ...
    pub fn show_project_detail(&mut self, cx: &mut Context<Self>) { self.project_detail_open = true; cx.notify(); }
    pub fn hide_project_detail(&mut self, cx: &mut Context<Self>) { self.project_detail_open = false; cx.notify(); }
    ...
    fn open_edit_quotation_form(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.edit_form.is_some() { return; }
        let (qid, ref_num, project_id, notes, extras) = {
            let store = self.store.read(cx);
            let Some(sid) = store.selected_id else { return; };
            let Some(q)   = store.quotations.iter().find(|q| q.id == sid).cloned() else { return; };
            let extras    = store.selected_extras.clone().unwrap_or_default();
            (sid, q.reference_number, q.project_id, q.notes, extras)
        };
        let form = cx.new(|cx| QuotationForm::edit_for(
            self.store.clone(), qid, ref_num, &extras, notes, Some(project_id), cx,
        ));
        let fh  = form.read(cx).focus_handle(cx);
...
impl Render for QuotationPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let selected_quotation = if detail_open && active_tab == Tab::Quotations {
            let store = self.store.read(cx);
            store.selected_id.and_then(|sid| {
                store.quotations.iter().find(|q| q.id == sid).cloned()
            })
        } else {
            None
        };
        ...
        let content = if detail_open && active_tab == Tab::Quotations {
            let detail_sidebar = {
                    ...
                    .child(
                        div()
                            ...
                            .child(div().flex_1().text_size(rems(0.923)).text_color(rgb(c.text_default))
                                .font_weight(gpui::FontWeight::BOLD).child("Quotation Details"))
                            .child(
                                div()
                ...
                if let Some(q) = selected_quotation {
                    ...
                    sidebar = sidebar.child(
                        div().id("quot-detail-scroll").flex_1().min_h(px(0.)).overflow_y_scroll().pb(px(64.))
                            .flex().flex_col()
                            .child(quot_detail_field("Reference", q.reference_number.clone(), &c))
                            ...
                            .child(quot_detail_field("Total (USD)", format!("${:.2}", q.total_usd), &c))
                            .child(quot_detail_field("Created", q.created_at.get(..10).unwrap_or("").to_string(), &c))
                            .child(quot_detail_field("Notes", q.notes.clone().unwrap_or_else(|| "—".into()), &c))
                    )
                    .child(
                        div()
                            .px(px(12.)).py(px(10.))
                            ...
                            .child({
                                let mut btn = div()
                                    ...
                                    .bg(rgb(if can_edit { c.surface_active } else { c.surface_default }))
            ...
            let proj_sidebar = {
                ...
                if let Some(p) = selected_project {
                    ...
                    sidebar = sidebar.child(
                        div().id("quot-proj-detail-scroll").flex_1().min_h(px(0.)).overflow_y_scroll().pb(px(64.))
                            .flex().flex_col()
                            .child(quot_detail_field("Name",           p.name.clone(), &c))
                            .child(quot_detail_field("Client",         p.client_name.clone(), &c))
                            .child(quot_detail_field("Address",        p.client_address.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Attn",           p.client_attn.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Tel",            p.client_tel.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Client Contact", p.client_contact.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("VASSL Contact",  p.vassl_contact.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Status",         status_label, &c))
                            .child(quot_detail_field("Date Started",   p.date_started.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Date Completed", p.date_completed.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Signed-off",     p.signedoff_date.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Technicians",    p.technicians.clone().unwrap_or_else(|| "—".into()), &c))
                            .child(quot_detail_field("Created",        p.created_at.get(..10).unwrap_or("").to_string(), &c))
                    );
                } else {
                    sidebar = sidebar.child(
                        div().flex_1().flex().items_center().justify_center()
...
fn quot_detail_field(label: impl Into<String>, value: impl Into<String>, c: &vassl_ui::ThemeColors) -> impl gpui::IntoElement {
    div()
