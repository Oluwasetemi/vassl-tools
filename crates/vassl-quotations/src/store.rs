...
impl QuotationStore {
    ...
    pub fn load_quotations(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let quot_task = cx
                .background_executor()
                .spawn(async move { db.list_quotations_with_project() });
            let proj_task = cx
                .background_executor()
                .spawn(async move { db2.list_projects() });
    ...
    pub fn select_quotation(&mut self, id: i64, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let extras = cx
                .background_executor()
                .spawn(async move { db2.get_quotation_extras(id) })
                .await;
            let _ = this.update(cx, |store, cx| {
                match items {
                    Ok(i) => {
                        store.line_items = i;
                        cx.emit(QuotationEvent::ItemsLoaded);
                    }
                    Err(e) => tracing::error!("list_items_for_quotation failed: {e:?}"),
                }
                match extras {
                    Ok(e) => {
                        store.selected_extras = Some(e);
                    }
    ...
    pub fn select_next(&mut self, cx: &mut Context<Self>) -> Option<usize> {
        ...
        let next = match cur {
            None => 0,
            Some(i) => (i + 1).min(self.quotations.len() - 1),
        };
    ...
    pub fn set_quotation_context_menu(
        &mut self,
        target: QuotationContextMenu,
        cx: &mut Context<Self>,
    ) {
        self.context_menu_quotation = Some(target);
        cx.notify();
    }
    pub fn clear_quotation_context_menu(&mut self, cx: &mut Context<Self>) {
        self.context_menu_quotation = None;
        cx.notify();
    }
    pub fn set_project_context_menu(&mut self, target: ProjectContextMenu, cx: &mut Context<Self>) {
        self.context_menu_project = Some(target);
        cx.notify();
    }
    pub fn clear_project_context_menu(&mut self, cx: &mut Context<Self>) {
        self.context_menu_project = None;
        cx.notify();
    }
...
mod tests {
    ...
    fn status_badge_color_mapping() {
        assert_eq!(
            status_badge_color(QuotationStatus::Draft),
            crate::colors::STATUS_GREY
        );
        assert_eq!(
            status_badge_color(QuotationStatus::Sent),
            crate::colors::STATUS_AMBER
        );
        assert_eq!(
            status_badge_color(QuotationStatus::Accepted),
            crate::colors::STATUS_GREEN
        );
        assert_eq!(
            status_badge_color(QuotationStatus::Rejected),
            crate::colors::STATUS_RED
        );
    }
}
