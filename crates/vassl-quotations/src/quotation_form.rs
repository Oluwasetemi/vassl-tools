...
impl QuotationForm {
    pub fn new(
        ...
        let _store_sub = cx.observe(&store, |this, store_entity, cx| {
            ...
            this.project_dropdown
                .update(cx, |d, cx| d.set_items(items, loading, cx));
        });

        let db = vassl_db::AppDatabase::global(&**cx);
        let rate_default = vassl_db::shared::get_setting(db, "pricebook.usd_to_jmd_rate")
            .ok()
            .flatten()
            .unwrap_or_else(|| "157.50".into());
        let gct_default = vassl_db::shared::get_setting(db, "quotations.tax_rate")
            .ok()
            .flatten()
            .unwrap_or_else(|| "15.0".into());
        let notes_default = vassl_db::shared::get_setting(db, "quotations.notes_template")
            .ok()
            .flatten()
            .unwrap_or_default();
        ...
        Self {
            ...
            notes: cx.new(move |cx| {
                ...
                if !notes_default.is_empty() {
                    f.set_text(&notes_default, cx);
                }
                f
            }),
    ...
    pub fn edit_for(
        ...
        let settings_rate: f64 = vassl_db::shared::get_setting(db, "pricebook.usd_to_jmd_rate")
            ...
            .unwrap_or(157.50);
        let effective_rate =
            if extras.exchange_rate_jmd > 0.0 && (extras.exchange_rate_jmd - 156.0).abs() > 0.01 {
            ...
            None => {
                ...
                if let Some(qid) = editing_id {
                    cx.spawn(async move |this, cx| {
                        let result = db
                            .update_quotation_financials(qid, pid, notes, rate, disc, gct, days)
                            .await;
...
impl Focusable for QuotationForm {
    fn focus_handle(&self, cx: &gpui::App) -> FocusHandle {
        self.notes.read(cx).focus_handle.clone()
    }
}
...
mod tests {
    ...
    fn reference_number_format() {
        let r = "VASSL-2026-0001";
        assert!(r.starts_with("VASSL-"));
        assert_eq!(r.len(), 15);
    }
    #[test]
    fn form_requires_project() {
        assert!(validate_form(None).is_some());
    }
    #[test]
    fn form_valid_with_project() {
        assert!(validate_form(Some(1)).is_none());
    }
    #[test]
    fn projects_to_items_maps_correctly() {
