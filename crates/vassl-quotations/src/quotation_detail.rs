...
impl Render for QuotationDetail {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let effective_rate =
            if extras.exchange_rate_jmd > 0.0 && extras.exchange_rate_jmd != LEGACY_DEFAULT_RATE {
                            ...
                            .on_mouse_down(
                                ...
                                move |_, _, cx: &mut gpui::App| {
                                    store.update(cx, |s, cx| {
                                        s.transition_status(id, ns.clone(), cx)
                                    });
                                },
                            )
...
fn totals_footer(
    ...
    div()
        ...
        .child(total_row(
            "Sub-total",
            &format!("US$ {subtotal_usd:.2}"),
            c.text_muted,
            c,
        ))
        .when(disc_pct > 0.0, |d| {
            d.child(total_row(
                &format!("Less discount ({disc_pct:.1}%)"),
                &format!("- US$ {discount_usd:.2}"),
                c.status_amber,
                c,
            ))
            .child(total_row(
        ...
        .child(total_row(
            "Grand Total",
            &format!("US$ {grand_usd:.2}"),
            c.status_green,
            c,
        ))
        .child(total_row(
            &format!("Grand Total (J$ @ {rate:.0})"),
            &format!("J$ {}", fmt_jmd(grand_jmd)),
            c.status_green,
            c,
        ))
        ...
        .child(total_row(
            "Deposit on order (50%)",
            &format!("J$ {}", fmt_jmd(deposit_jmd)),
            c.text_muted,
            c,
        ))
        .child(total_row(
            "Balance on completion",
            &format!("J$ {}", fmt_jmd(balance_jmd)),
            c.text_muted,
            c,
        ))
        .when(!validity.is_empty(), |d| {
            d.child(
                div()
                    .mt(px(4.))
...
mod tests {
    ...
    fn next_status_transitions_are_correct() {
        assert_eq!(
            next_transitions(QuotationStatus::Draft),
            vec![QuotationStatus::Sent]
        );
        assert_eq!(
            next_transitions(QuotationStatus::Sent),
            vec![QuotationStatus::Accepted, QuotationStatus::Rejected]
        );
    ...
    fn transition_label_is_human_readable() {
        ...
        assert_eq!(
            transition_label(&QuotationStatus::Accepted),
            "Mark as Accepted"
        );
        assert_eq!(
            transition_label(&QuotationStatus::Rejected),
            "Mark as Rejected"
        );
    }
