...
fn read_pb_rows(src: &Connection) -> anyhow::Result<Vec<PbRow>> {
    ...
    for &(table, has_product_group, is_install_material) in PB_TABLES {
        ...
        for (manufacturer, description, part_number, cost_usd, duty, markup, selling, group) in rows
        {
            let name = match description {
                Some(ref d) if !d.trim().is_empty() && !is_header_row(d) => d.trim().to_string(),
                _ => continue,
            };
            let manufacturer = manufacturer.filter(|m| !m.trim().is_empty() && !is_header_row(m));
...
fn read_stock_map(src: &Connection) -> anyhow::Result<HashMap<String, StockInfo>> {
    ...
    for &table in SC_TABLES {
        let mut stmt = src
            .prepare(&format!(
                "SELECT part_number, stock_qty_2025, model_number FROM {table} WHERE id > 1"
            ))
        ...
        for (part, qty, model) in rows {
            if let Some(p) = part {
                ...
                map.entry(p)
                    .and_modify(|e| {
                        e.qty += q;
                        if e.model_number.is_none() {
                            e.model_number = model.clone();
                        }
                    })
                    .or_insert(StockInfo {
                        qty: q,
...
fn import(src: &Connection, dst: &Connection, dry_run: bool) -> anyhow::Result<ImportSummary> {
    ...
    for row in &rows {
        ...
        // ── product ───────────────────────────────────────────────────────
        let existing_pid: Option<i64> = dst
            .query_row(
                ...
                if stock.qty > 0.0 {
                    ...
                    if !dry_run {
                        dst.execute(
                            ...
                            params![product_id, stock.qty, unit_cost, row.manufacturer, now],
                        )?;
                    }
...
fn main() -> anyhow::Result<()> {
    ...
    if dry_run {
        ...
        for r in rows.iter().take(10) {
            ...
            println!(
                "  sku={sku:35}  cost={:?}  duty={:.1}%  markup={:.0}%",
                r.cost_price_usd,
