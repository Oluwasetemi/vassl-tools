...
pub fn parse_xlsx(path: &Path) -> anyhow::Result<Vec<ImportRow>> {
    let mut wb: Xlsx<_> =
        open_workbook(path).with_context(|| format!("could not open {}", path.display()))?;
    ...
    for sheet_name in sheet_names {
        ...
        let headers: Vec<String> = match rows_iter.next() {
            Some(r) => r
                .iter()
                .map(|c| cell_str(c).unwrap_or_default().to_uppercase())
                .collect(),
            None => {
                tracing::warn!("sheet {sheet_name:?} is empty, skipping");
                continue;
            }
        };
        ...
        let i_name = match col_any(&headers, columns::NAME) {
            ...
            None => {
                tracing::warn!(
                    "sheet {sheet_name:?}: no name column found, skipping (tried {:?})",
                    columns::NAME
                );
                continue;
            }
        ...
        for (row_idx, row) in rows_iter.enumerate() {
            ...
            out.push(ImportRow {
                ...
                currency: get_str(row, i_curr)
                    .unwrap_or_else(|| columns::DEFAULT_CURRENCY.to_string()),
    ...
    if out.is_empty() {
        bail!(
            "no importable rows found — check that sheets contain one of: {:?}",
            columns::NAME
        );
    }
    Ok(out)
