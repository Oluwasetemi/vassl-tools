...
impl InventoryDb {
    ...
    pub fn list_products(&self) -> anyhow::Result<Vec<Product>> {
        self.select::<(i64, String, String, Option<String>, String, f64, Option<String>, Option<String>, Option<i64>, String)>(
            "SELECT id, sku, name, category, unit, min_stock_level, description, notes,
                    preferred_supplier_id, created_at
             FROM products ORDER BY name",
        )
        .context("prepare list_products")?()
        .context("execute list_products")
        .map(|rows| {
            rows.into_iter().map(|(id, sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, created_at)| {
                Product { id, sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, created_at }
            }).collect()
        })
    }
    ...
    pub fn list_products_with_stock(&self) -> anyhow::Result<Vec<(Product, f64)>> {
        self.select::<(i64, String, String, Option<String>, String, f64, Option<String>, Option<String>, Option<i64>, String, f64)>(
        ...
        .map(|rows| {
            rows.into_iter().map(|(id, sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, created_at, current_stock)| {
                (Product { id, sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, created_at }, current_stock)
            }).collect()
        })
    }
    ...
    pub async fn insert_product(
        ...
        self.write(move |conn| {
            conn.exec_bound::<(String, String, Option<String>, String, f64, Option<String>, Option<String>, Option<i64>, String)>(
                "INSERT INTO products (sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, created_at)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
            )
            .context("prepare insert_product")?
            ((sku, name, category, unit, min_stock_level, description, notes, preferred_supplier_id, now))
    ...
    pub async fn update_product(
        ...
        self.write(move |conn| {
            conn.exec_bound::<(String, Option<String>, String, f64, Option<String>, Option<i64>, i64)>(
                "UPDATE products
            ...
            ((name, category, unit, min_stock_level, description, preferred_supplier_id, id))
...
mod tests {
    ...
    async fn insert_and_list_product() {
        ...
        let id = db.insert_product("CAM-001", "IP Camera", Some("CCTV"), "pcs", 5.0, None, None, None).await.unwrap();
        assert!(id > 0);
        let products = db.list_products().unwrap();
    ...
    async fn current_stock_zero_when_no_entries() {
        ...
        let id = db.insert_product("NVR-001", "NVR", None, "pcs", 2.0, None, None, None).await.unwrap();
    ...
    async fn insert_stock_entry_updates_current_stock() {
        ...
        let id = db.insert_product("CAB-001", "Cable", None, "meters", 100.0, None, None, None).await.unwrap();
    ...
    async fn products_below_min_stock_detected() {
        ...
        let id = db.insert_product("DVR-001", "DVR", None, "pcs", 5.0, None, None, None).await.unwrap();
    ...
    async fn products_at_zero_min_not_alerted() {
        ...
        db.insert_product("MISC-001", "Misc", None, "pcs", 0.0, None, None, None).await.unwrap();
        let below = db.products_below_min_stock().unwrap();
    ...
    async fn list_products_with_stock_aggregates_correctly() {
        ...
        let id = db.insert_product("PTZ-001", "PTZ Camera", None, "pcs", 2.0, None, None, None).await.unwrap();
    ...
    async fn description_round_trips_through_insert_and_list() {
        ...
        let id = db.insert_product(
            ...
            Some("Wide-angle lens, 24mm"), None, None,
        ).await.unwrap();
        assert!(id > 0);
        let products = db.list_products().unwrap();
    ...
    async fn update_product_persists_changes() {
        ...
        let id = db.insert_product("CAM-001", "IP Camera", None, "pcs", 5.0, None, None, None).await.unwrap();
        db.update_product(id, "IP Camera HD", Some("CCTV"), "pcs", 10.0, Some("Updated"), None).await.unwrap();
        let products = db.list_products().unwrap();
    ...
    async fn preferred_supplier_id_round_trips() {
        ...
        let id = db.insert_product("CAM-002", "Fisheye Camera", None, "pcs", 0.0, None, None, Some(42)).await.unwrap();
        let products = db.list_products().unwrap();
        assert_eq!(products[0].preferred_supplier_id, Some(42));
        db.update_product(id, "Fisheye Camera", None, "pcs", 0.0, None, None).await.unwrap();
        let products = db.list_products().unwrap();
